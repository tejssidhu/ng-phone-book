...
export class ContactService {
    ...
    getContacts(userId: string): Observable<IContact[]> {
        return this.http.get(myGlobals.serviceRootUrl + 'Users(' + userId + ')' + '/Phonebook.MyContacts').pipe(
    ...
    getContact(id: string): Observable<IContact> {
        return this.http.get(myGlobals.serviceRootUrl + 'Contacts(' + id + ')').pipe(
    ...
    createContact(contact: IContact): Observable<IContact> {
        return this.http.post(myGlobals.serviceRootUrl + 'Contacts', JSON.stringify(contact), httpOptions).pipe(
            tap((contact: IContact) => console.log(`contact created: userId=${contact.id}`)),
    ...
    updateContact(contact: IContact): Observable<IContact> {
        return this.http.put(myGlobals.serviceRootUrl + 'Contacts(' + contact.id + ')', JSON.stringify(contact), httpOptions).pipe(
            tap((contact: IContact) => console.log(`contact updated: userId=${contact.id}`)),
            catchError(this.handleError<any>('createContact'))
        );
    }
    //let contact = <IContact>response.json();
